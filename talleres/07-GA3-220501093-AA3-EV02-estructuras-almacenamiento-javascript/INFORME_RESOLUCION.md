...
- **Entrega pública:** un PDF genérico, regenerable y sin datos personales.
- **Entrega para SENA:** un único PDF personalizado dentro de
...
en las páginas físicas 9 y 10, exige resolver cuatro problemas con JavaScript. El documento
PDF integra el análisis, las validaciones, el código completo, la matriz de cumplimiento y
los resultados de las pruebas en un producto único y fácil de revisar.
...
- `03_entrega` es una zona exclusivamente pública y contiene el PDF de GitHub.
- `04_entrega_personalizada.local` aparece solo en el equipo local y contiene exactamente
  un archivo PDF identificado: ese es el que debe seleccionarse al entregar en la plataforma SENA.
...
- El DOCX personalizado se crea únicamente en un directorio temporal para exportar el PDF y
...
En un clon sin perfil se genera únicamente el PDF público. Cuando existe el perfil local,
se crea además el único PDF personalizado listo para entregar.
